...
def extensions = Jenkins.getInstanceOrNull().getExtensionList(Maven.DescriptorImpl.class)[0]
...
properties.maven.each { mavenTool ->
    ...
    def installer = new ZipExtractionInstaller(mavenTool.value.label as String, mavenTool.value.url as String, mavenTool.value.subdir as String);

    describableList.add(new InstallSourceProperty([installer]))

    installations.add(new MavenInstallation(mavenTool.value.name as String, "", describableList))
}
